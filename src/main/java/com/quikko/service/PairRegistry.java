 ...
 * The active {@code MatchQueueStore} pair is deleted the instant a match
 * ends (Skip/Stop/Report), but a Time Capsule can still be left right after
 * that — and since anonymous ids are never shared with the other browser,
 * the server is the only place that can resolve "who was I just talking
 * to" for capsule addressing. Entries are short-lived and exist purely for
 * that purpose.
 */
@Service
public class PairRegistry {
    ...
    /**
     * Returns the other participant's anonymous id for this pairing, if the
     * given anonId was actually part of it and the record hasn't expired.
     */
