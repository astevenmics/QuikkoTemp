...
/**
 * Time Capsule: an optional note left for the specific stranger you were just matched with.
 * Double opt-in — it only ever unlocks and is retrievable if BOTH sides left one for each other in that pairing, and only after the 7-day unlock window.
 * No usernames/contact info are ever exchanged; everything is addressed purely by anonymous session id and retrieved purely by a random claim token.
 */
@Service
public class CapsuleService {
    ...
    /**
     * Leaves (or re-returns the existing token for) a capsule addressed to whoever {@code senderAnonId} was just paired with in {@code pairId}.
     * Returns empty if the pairing can no longer be resolved (e.g. too much time has passed since the match ended).
     */
    @Transactional
