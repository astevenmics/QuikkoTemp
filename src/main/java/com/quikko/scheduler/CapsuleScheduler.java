...
/**
 * Daily job that finds pairings where BOTH sides left a Time Capsule for
 * each other and the 7-day unlock window has passed, and marks them
 * delivered. (The claim page in CapsuleService also computes this live on
 * demand, so a user isn't stuck waiting for the nightly run — this job
 * exists per the double-opt-in unlock design and is the natural place to
 * hook in a push/email notification if Quikko ever grows accounts.)
 */
@Component
